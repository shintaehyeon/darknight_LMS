...
* **[KR] 해결 매커니즘**:
  - **DNR 규칙 1003/1004/1005/1006 동적 CORS 우회 기법 도입**: 네이버 CDN(`naverncp.com`) 및 플레이어 도메인(`hducc.handong.edu`) 요청 시, 요청을 보낸 오리진(메인 프레임 `lms.handong.edu` 또는 플레이어 iframe `hducc.handong.edu`)에 따라 맞춤형 CORS 헤더(`Access-Control-Allow-Origin` 및 `Access-Control-Allow-Credentials: true`)를 반환하도록 하는 DNR 동적 규칙들을 새롭게 등록했습니다. 이를 통해 모든 오리진 환경에서 브라우저의 CORS 검증을 완벽하게 통과할 수 있게 되었습니다.
  - **메인 프레임(frameId: 0) 직접 다운로드 위임 엔진 구축 (메모리 유실 및 샌드박스 장벽 격파)**: 기존에는 iframe 내부의 `content.js`에서 fetch를 실행하고 수백 MB의 대용량 비디오 바이너리(`ArrayBuffer`)를 `chrome.runtime.sendMessage`를 거쳐 메인 프레임으로 릴레이 전송했습니다. 이 과정에서 브라우저의 구조화 복사(Structured Clone) 크기 한계로 인해 데이터가 유실/손상되어 비디오 파일이 깨지고 재생되지 않는 문제가 발생했습니다. 또한 iframe 내부는 다운로드가 차단되는 Sandbox 규제가 있습니다. 이를 해결하기 위해, 모든 다운로드 요청(HLS 및 MP4)을 **최상위 메인 프레임(`frameId: 0`)으로 즉시 전달하여 처음부터 메인 프레임의 컨텐트 스크립트가 직접 fetch하도록 설계**했습니다. 메인 프레임은 Sandbox 제한이 전혀 없으므로 대용량 이진 바이너리를 다른 스크립트로 전달하는 과정 없이 곧바로 가상 앵커를 띄워 로컬 PC로 깨끗하게 파일을 저장함으로써 100% 온전한 MP4/TS 원본을 완벽히 획득하는 데 성공했습니다.
...
* **[EN] Resolution**:
  - **Persistent DNR CORS Bypassing (Rules 1003-1006)**: Registered multiple declarativeNetRequest rules with different priorities and `initiatorDomains` checks to dynamically append CORS response headers (`Access-Control-Allow-Origin: https://lms.handong.edu` or `https://hducc.handong.edu`, and `Access-Control-Allow-Credentials: true`) depending on which frame initiated the request. This eliminates browser CORS blockages for both embedded and standalone page fetches.
